...
export default async function jestGetTestsList(
  ...
  installHooks(environment, (name, _describe, _fn) => {
    context.children.push({ type: 'hook', ancestors: [...context.ancestors, context.describe], callPath: name });
  })
  ...
  installTest(environment, (name, describe, _fn) => {
    const ancestors = context ? [...context.ancestors, context.describe] : [];
    context.children.push({ type: 'test', ancestors, describe, callPath: name });
    testResults.push({
  ...
  if(!testResults.length){
    // Workaround for Jest not complaining about no tests found
